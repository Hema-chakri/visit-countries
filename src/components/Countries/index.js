      ...
      <div className="app-container">
        ...
        <ul className="country-items">
          {countriesList.map(eachCountry => (
            <li className="countryName-visit-container">
              <p>{eachCountry.name}</p>
              <button
                type="button"
                onClick={() => this.onCountryVisit(eachCountry.id)}
              >
                <p>{eachCountry.isVisited === true ? `Visited` : `Visit`}</p>
              </button>
            </li>
          ))}
        </ul>
        ...
        {visitedCountries.length > 0 ? (
          <ul className="country-details">
            {countriesList
              ...
              .map(eachCountryVisit => (
                <VisitedCountries
                  visitedCountryDetails={eachCountryVisit}
                  key={eachCountryVisit.id}
